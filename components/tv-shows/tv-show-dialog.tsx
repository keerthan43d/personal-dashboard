...
const GENRES = ["Action","Animation","Comedy","Crime","Documentary","Drama","Fantasy","Horror","Mystery","Romance","Sci-Fi","Thriller","Reality","Talk Show"];
...
export function TvShowDialog({ open, onClose, existing }: TvShowDialogProps) {
  const { add, edit } = useTvShows();
  ...
  const [form, setForm] = useState({
    ...
    seasons:   existing?.seasons?.toString()  ?? "",
    episodes:  existing?.episodes?.toString() ?? "",
  ...
  async function searchTMDB() {
    ...
    try {
      ...
      if (!key) {
        toast.info("Add NEXT_PUBLIC_TMDB_KEY to .env.local for auto posters.");
        setSearching(false);
        return;
      }
      const res  = await fetch(
        `https://api.themoviedb.org/3/search/tv?api_key=${key}&query=${encodeURIComponent(form.title)}&first_air_date_year=${form.year}`
      );
      const data = await res.json();
      const hit  = data.results?.[0];
      if (!hit) { toast.error("Show not found on TMDB"); return; }
      setForm((f) => ({
        ...f,
        posterUrl: hit.poster_path ? `https://image.tmdb.org/t/p/w500${hit.poster_path}` : f.posterUrl,
        year:      hit.first_air_date?.split("-")[0] ?? f.year,
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      if (existing) {
        ...
        await add(data);
        toast.success("Added to TV Shows");
      }
      onClose();
    } finally {
      setSaving(false);
      ...
      <DialogContent className="bg-[#161616] border-white/10 text-foreground max-w-lg max-h-[90vh] flex flex-col">
        <DialogHeader>
          <DialogTitle className="font-display text-lg">
            {existing ? "Edit TV Show" : "Add TV Show"}
          </DialogTitle>
        </DialogHeader>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col flex-1 overflow-hidden mt-1">
            ...
            <div className="flex-1 space-y-3">
              <div className="space-y-1.5">
                ...
                <div className="flex gap-2">
                  <Input value={form.title} onChange={set("title")} placeholder="Breaking Bad"
              ...
              <div className="space-y-1.5">
                ...
                <Input value={form.creator} onChange={set("creator")} placeholder="Vince Gilligan"
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-1.5">
              ...
              <Select value={form.status} onValueChange={(v) => setForm((f) => ({ ...f, status: v as TvShow["status"] }))}>
            ...
            <div className="space-y-1.5">
              ...
              <Input value={form.year} onChange={set("year")} type="number" placeholder="2008"
                className="bg-white/5 border-white/10 h-9" />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs text-muted-foreground">Network</Label>
              <Input value={form.network} onChange={set("network")} placeholder="HBO, Netflix…"
                className="bg-white/5 border-white/10 h-9" />
            </div>

            <div className="space-y-1.5">
              <Label className="text-xs text-muted-foreground">Seasons</Label>
              <Input value={form.seasons} onChange={set("seasons")} type="number" placeholder="5"
                className="bg-white/5 border-white/10 h-9" />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs text-muted-foreground">Episodes</Label>
              <Input value={form.episodes} onChange={set("episodes")} type="number" placeholder="62"
                className="bg-white/5 border-white/10 h-9" />
            </div>
          ...
          <DialogFooter className="gap-2 pt-3 border-t border-white/8 mt-3">
            ...
            <Button type="submit" disabled={saving || !form.title.trim()}
              ...
              {saving ? "Saving…" : existing ? "Save Changes" : "Add Show"}
            </Button>
          </DialogFooter>
        </form>
      </DialogContent>
